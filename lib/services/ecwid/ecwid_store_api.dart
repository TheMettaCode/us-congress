...
class EcwidStoreApi {
  ...
  static Future<List<EcwidStoreItem>> getEcwidStoreProducts(
    ...
    try {
      ...
      logger.w(
          '[GET ECWID PRODUCTS FUNCTION] ERROR RETRIEVING ECWID STORE ITEMS DATA FROM DBASE (ECWID_STORE_API): $e ^^^^^');
    ...
    if (localCurrentEcwidProductsList.isEmpty ||
      ...
      try {
        ...
        if (response.statusCode == 200) {
          logger.d(
              '[GET ECWID PRODUCTS FUNCTION] ECWID PRODUCT RETRIEVAL SUCCESS! *****');
          ...
          if (ecwidStore.items.isNotEmpty) {
            ...
            try {
              ...
              logger.i(
                  '[GET ECWID PRODUCTS FUNCTION] NEW ECWID PRODUCTS SAVED TO DBASE *****');
            } catch (e) {
              logger.w(
                  '[GET ECWID PRODUCTS FUNCTION] ERROR SAVING ECWID STORE ITEMS DATA TO DBASE (ECWID_STORE_API): $e ^^^^^');
              userDatabase.put('ecwidProducts', {});
            }
          }
          ...
          logger.w(
              '[GET ECWID PRODUCTS FUNCTION] API ERROR: LOADING ECWID PRODUCTS FROM DBASE: ${response.statusCode} *****');

          return localCurrentEcwidProductsList.isNotEmpty
              ? localCurrentEcwidProductsList
              : [];
        }
      } on TimeoutException catch (e) {
        debugPrint(
            '[GET ECWID PRODUCTS FUNCTION] API TIMEOUT ERROR: FETCHING PRODUCTS: $e');
        ...
        debugPrint(
            '[GET ECWID PRODUCTS FUNCTION] API ERROR FETCHING PRODUCTS: $e');
      ...
      logger.d(
          '[GET ECWID PRODUCTS FUNCTION] ECWID PRODUCTS NOT UPDATED: CATALOG IS CURRENT *****');
      // userDatabase.put('lastEcwidProductsRefresh', '${DateTime.now()}');

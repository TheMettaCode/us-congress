...
class TwitterServiceApi {
  ...
  static final String capitolBabbleConsumerKey =
      dotenv.env['CAPITOLBABBLE_TWITTER_API_KEY'];
  static final String capitolBabbleConsumerSecret =
      dotenv.env['CAPITOLBABBLE_TWITTER_SECRET_KEY'];
  static final String capitolBabbleBearerToken =
      dotenv.env['CAPITOLBABBLE_TWITTER_BEARER_TOKEN'];
  ...
  static final String capitolBabbleClientId =
      dotenv.env['CAPITOLBABBLE_TWITTER_CLIENT_ID'];
  static final String capitolBabbleClientSecret =
      dotenv.env['CAPITOLBABBLE_TWITTER_CLIENT_SECRET'];
      ...
      try {
        ...
        if (assetImageUrl != null) {
          ...
          final uploadedMedia = await twitter.media.uploadMedia(
            ...
            onFailed: (error) =>
                logger.d('Upload failed due to "${error.message}"'),
          );

          //! You can easily post a tweet with uploaded media.
          await twitter.tweets.createTweet(
              text: message,
              media: v2.TweetMediaParam(mediaIds: [uploadedMedia.data.id]));
        } else {
